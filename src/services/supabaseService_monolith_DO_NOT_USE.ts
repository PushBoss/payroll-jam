...
export const supabaseService = {
  ...
  // ⛔ DECOMMISSIONED — Do NOT use this method.
  // The SERVICE_ROLE_KEY has been removed from all frontend env files.
  // All privileged operations must go through the `admin-handler` Edge Function:
  //   supabase.functions.invoke('admin-handler', { body: { action, payload } })
  // If you see this warning in the console, you have a stray getAdminClient() call that needs migrating.
  getAdminClient: async (): Promise<any> => {
    console.error(
      '⛔ getAdminClient() called but is DECOMMISSIONED. ' +
      'Use supabase.functions.invoke("admin-handler", ...) instead. ' +
      'Trace the call stack and migrate this call to the Edge Function.'
    );
    return null;
  },
