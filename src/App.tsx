...
function AppContent() {
  ...
  useEffect(() => {
    async function loadPlansFromBackend() {
      ...
      try {
        const globalConfig = await CompanyService.getGlobalConfig();
  ...
  const handleUpdatePlans = async (updatedPlans: PricingPlan[]) => {
    ...
    if (isSupabaseMode) {
      ...
      try {
        ...
        const verifyConfig = await CompanyService.getGlobalConfig();
  ...
  const handleDeletePayRun = async (runId: string) => {
    if (isSupabaseMode && user?.companyId) {
      try {
        ...
        console.error('Error deleting pay run:', e);
  ...
  const handleSaveLeaveRequest = async (req: LeaveRequest) => {
    ...
    if (isSupabaseMode && user?.companyId) await EmployeeService.saveLeaveRequest(req, user.companyId);
  };
  const handleUpdateLeaveStatus = async (id: string, status: 'APPROVED' | 'REJECTED', dates?: string[]) => {
    ...
    if (isSupabaseMode && user?.companyId) {
      ...
      if (target) await EmployeeService.saveLeaveRequest(target, user.companyId);
    }
  };
  const handleUpdateCompany = async (data: CompanySettings) => {
    ...
    if (isSupabaseMode && user?.companyId) await CompanyService.saveCompany(user.companyId, updatedData);
  };
  ...
  const handleUpdateDepartments = async (newDepts: Department[]) => {
    ...
    if (companyData) {
      ...
      if (isSupabaseMode && user?.companyId) await CompanyService.saveCompany(user.companyId, updated);
    }
  };
  ...
  const handleEmployeeAccountSetup = async (password: string) => {
    ...
    try {
      ...
      if (isSupabaseMode) {
        ...
        const updatedEmployee = { ...employee, userId: newUser.id, isOnboarded: true };
  ...
  if (!user) {
    ...
    return (
      <Suspense fallback={<LoadingFallback />}>
        ...
        {currentPath === 'pricing' && <Pricing onSignup={(plan, cycle) => { setSelectedPlan(plan); setSelectedCycle(cycle); navigateTo('signup'); }} onLogin={() => navigateTo('login')} onBack={() => navigateTo('home')} onFeaturesClick={() => navigateTo('features')} onFaqClick={() => navigateTo('faq')} onContactClick={() => navigateTo('contact-us')} onPrivacyClick={() => navigateTo('privacy-policy')} onTermsClick={() => navigateTo('terms-of-service')} plans={plans} />}
        {currentPath === 'features' && <Features onSignup={() => { setSelectedPlan('Starter'); setSelectedCycle('monthly'); navigateTo('signup'); }} onLogin={() => navigateTo('login')} onBack={() => navigateTo('home')} onPricingClick={() => navigateTo('pricing')} onFaqClick={() => navigateTo('faq')} onContactClick={() => navigateTo('contact-us')} onPrivacyClick={() => navigateTo('privacy-policy')} onTermsClick={() => navigateTo('terms-of-service')} />}
        {currentPath === 'faq' && <FAQ onSignup={() => { setSelectedPlan('Starter'); setSelectedCycle('monthly'); navigateTo('signup'); }} onLogin={() => navigateTo('login')} onBack={() => navigateTo('home')} onPricingClick={() => navigateTo('pricing')} onFeaturesClick={() => navigateTo('features')} onContactClick={() => navigateTo('contact-us')} onPrivacyClick={() => navigateTo('privacy-policy')} onTermsClick={() => navigateTo('terms-of-service')} onContactSupport={() => handleContactSupport('faq')} />}
        {currentPath === 'privacy-policy' && <PrivacyPolicy onBack={() => navigateTo('home')} onFeaturesClick={() => navigateTo('features')} onPricingClick={() => navigateTo('pricing')} onFaqClick={() => navigateTo('faq')} onContactClick={() => navigateTo('contact-us')} onLogin={() => navigateTo('login')} onSignup={() => { setSelectedPlan('Starter'); setSelectedCycle('monthly'); navigateTo('signup'); }} onPrivacyClick={() => navigateTo('privacy-policy')} onTermsClick={() => navigateTo('terms-of-service')} />}
        {currentPath === 'terms-of-service' && <TermsOfService onBack={() => navigateTo('home')} onFeaturesClick={() => navigateTo('features')} onPricingClick={() => navigateTo('pricing')} onFaqClick={() => navigateTo('faq')} onContactClick={() => navigateTo('contact-us')} onLogin={() => navigateTo('login')} onSignup={() => { setSelectedPlan('Starter'); setSelectedCycle('monthly'); navigateTo('signup'); }} onPrivacyClick={() => navigateTo('privacy-policy')} onTermsClick={() => navigateTo('terms-of-service')} />}
        {currentPath === 'home' && <LandingPage plans={plans} onLogin={() => navigateTo('login')} onSignup={(plan) => { setSelectedPlan(plan || 'Free'); setSelectedCycle('monthly'); navigateTo('signup'); }} onPricingClick={() => navigateTo('pricing')} onFeaturesClick={() => navigateTo('features')} onFaqClick={() => navigateTo('faq')} onContactClick={() => navigateTo('contact-us')} onPrivacyClick={() => navigateTo('privacy-policy')} onTermsClick={() => navigateTo('terms-of-service')} />}
  ...
  const renderPage = () => {
    ...
    switch (currentPath) {
      ...
      case 'reports': return <Reports history={payRunHistory} companyData={companyData!} onUpdatePayRun={handleSavePayRun} onDeletePayRun={handleDeletePayRun} onNavigate={navigateTo} employees={employees} integrationConfig={integrationConfig} />;
      case 'compliance':
      ...
      case 'sa-overview': case 'sa-tenants': case 'sa-billing': case 'sa-health': case 'sa-users': case 'sa-logs': case 'sa-settings': case 'sa-plans': case 'sa-pending-payments':
