...
// files to stay under Vercel's Hobby-plan 12-function-per-deployment limit.

const listPaymentMethods = async (req: VercelRequest, res: VercelResponse) => {
...
const setPrimaryPaymentMethod = async (req: VercelRequest, res: VercelResponse) => {
    try {
...
const removePaymentMethod = async (req: VercelRequest, res: VercelResponse) => {
    try {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (req.method === 'POST') {
        ...
        if (action === 'set-primary') return setPrimaryPaymentMethod(req, res);
        if (action === 'remove') return removePaymentMethod(req, res);
        return res.status(400).json({ error: 'body.action must be "set-primary" or "remove"' });
    }
