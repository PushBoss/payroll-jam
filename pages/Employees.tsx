...
export const Employees: React.FC<EmployeesProps> = ({ 
  ...
  const checkPlanLimit = (countToAdd = 1) => {
      const currentCount = employees.filter(e => e.status !== 'TERMINATED' && e.status !== 'ARCHIVED').length;
  ...
  const handleInvite = async (e: React.FormEvent) => {
      ...
      const onboardingToken = Math.random().toString(36).substring(7);
  ...
  const onSubmitAdd = (e: React.FormEvent) => {
      ...
      if (!isValidTRN(addForm.trn)) {
          toast.error("Invalid TRN. Must be 9 digits (e.g. 123-456-789)");
          return;
      }
      
      if (addForm.nis && !isValidNIS(addForm.nis) && addForm.nis !== 'PENDING') {
           toast.error("Invalid NIS. Must be Letter + 6 Digits (e.g. A123456)");
           return;
      }
  ...
  const saveProfileChanges = () => {
      if (selectedEmployee) {
          ...
          const updated = { ...selectedEmployee, status };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden animate-scale-in flex flex-col max-h-[90vh]">
                        ...
                        {['profile', 'financial', 'banking'].map((tab) => (
                            <button
                                ...
                                className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm capitalize ${
                                    editTab === tab ? 'border-jam-orange text-jam-black' : 'border-transparent text-gray-500 hover:text-gray-700'
                                }`}
                            >
                                {tab}
                            </button>
                ...
                <div className="p-6 overflow-y-auto">
                    ...
                    {editTab === 'financial' && (
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">TRN</label><input className="w-full border rounded p-2" value={selectedEmployee.trn} onChange={e => handleUpdateField('trn', e.target.value)} /></div>
                            <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">NIS</label><input className="w-full border rounded p-2" value={selectedEmployee.nis} onChange={e => handleUpdateField('nis', e.target.value)} /></div>
                    ...
                    {editTab === 'banking' && (
                        <div className="space-y-4">
                            ...
                            <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Account Number</label><input className="w-full border rounded p-2" value={selectedEmployee.bankDetails?.accountNumber} onChange={e => handleUpdateBank('accountNumber', e.target.value)} /></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden animate-fade-in flex flex-col max-h-[90vh]">
                ...
                <form onSubmit={onSubmitAdd} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">TRN (9 Digits)</label>
                        <input 
                            required 
                            type="text" 
                            placeholder="123-456-789"
                            className={`w-full border rounded-lg p-2 ${!isValidTRN(addForm.trn) && addForm.trn ? 'border-red-500' : 'border-gray-300'}`} 
                            value={addForm.trn} 
                            onChange={e => setAddForm({...addForm, trn: e.target.value})} 
                        />
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">NIS (Optional)</label>
                        <input 
                            type="text" 
                            placeholder="A123456"
                            className="w-full border border-gray-300 rounded-lg p-2" 
                    ...
                    <div>
                        ...
                        <input type="text" className="w-full border border-gray-300 rounded-lg p-2" value={addForm.accountNumber} onChange={e => setAddForm({...addForm, accountNumber: e.target.value})} />
                    </div>
