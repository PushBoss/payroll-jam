...
import { downloadFile, generateNCBFile, generateBNSFile, generateGLCSV } from '../utils/exportHelpers';
...
export const PayRun: React.FC<PayRunProps> = ({ 
    ...
    const [step, setStep] = useState<'SETUP' | 'REVIEW' | 'FINALIZE'>('SETUP');
    ...
    const handleImportCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files.length > 0) {
             toast.info("CSV Import feature coming soon. Using system data.");
             handleInitializeSystem();
        }
    };

    const handleDownloadTemplate = () => {
        const headers = "EmployeeID,GrossSalary,Bonus,Deductions";
        downloadFile("Payroll_Import_Template.csv", headers, "text/csv");
    };
                ...
                <div className="bg-white p-8 rounded-xl shadow-xl border border-gray-100 text-center">
                    ...
                    <div className="space-y-4">
                        <button 
                            ...
                            className={`w-full py-3.5 rounded-lg font-bold transition-all shadow-md flex justify-center items-center text-sm ${
                        ...
                        >
                            {isCalculating ? (
                                <span className="flex items-center"><Icons.Refresh className="w-4 h-4 mr-2 animate-spin" /> Calculating...</span>
                            ) : (
                                <span className="flex items-center"><Icons.Zap className="w-4 h-4 mr-2 text-jam-yellow" /> Calculate from system data</span>
                            )}
                        </button>
                        ...
                        <div className="relative">
                            ...
                            <button 
                                ...
                                disabled={isSuspended || isCalculating}
                                className={`w-full py-3.5 rounded-lg font-bold transition-all border flex justify-center items-center text-sm ${
                                    isSuspended 
                                    ? 'bg-gray-50 border-gray-200 text-gray-400 cursor-not-allowed'
                                    : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50 hover:border-gray-400'
                                }`}
                            >
                                <Icons.Upload className="w-4 h-4 mr-2 text-gray-500" /> Import from CSV
                            </button>
                        </div>
                    </div>
                    ...
                    <button 
                        ...
                        className="mt-6 text-xs text-gray-400 hover:text-jam-orange flex items-center justify-center mx-auto transition-colors"
                    >
                        <Icons.Download className="w-3 h-3 mr-1" /> Download CSV Template
                    </button>
                </div>
            </div>
        );
        ...
        return (
            <div className="space-y-6 animate-fade-in relative">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                    <div>
                        <div className="flex items-center space-x-3 mb-1"><h2 className="text-3xl font-bold text-gray-900">Review Pay Run: {payPeriod}</h2></div>
                        <div className="flex items-center space-x-2 text-sm"><span className="bg-jam-yellow/30 text-yellow-800 px-2 py-0.5 rounded font-bold uppercase text-xs">Draft Mode</span><span className="text-gray-500">• Review and adjust amounts before finalizing.</span></div>
                    </div>
                    <div className="flex items-center space-x-3 mt-4 md:mt-0">
                        <button onClick={() => setAddEmployeeModalOpen(true)} className="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 shadow-sm flex items-center text-sm font-medium"><Icons.Plus className="w-4 h-4 mr-2" /> Add Missing Employee</button>
                        <button onClick={() => { setStep('SETUP'); clearDraft(); }} disabled={isFinalizing} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg disabled:opacity-50 text-sm font-medium">Cancel</button>
                        <button onClick={handleFinalize} disabled={isFinalizing} className="bg-jam-orange text-jam-black px-6 py-2 font-bold rounded-lg hover:bg-yellow-500 shadow-lg flex items-center disabled:opacity-50 disabled:cursor-not-allowed text-sm">
                            {isFinalizing ? (
                                <span className="flex items-center"><Icons.Refresh className="w-4 h-4 mr-2 animate-spin"/> Processing...</span>
                            ) : (
